...
for nbIter=1:obj.params_nbMaxSteps
	...
	for n=1:M
		...
		s(n).BETA(:,s(n).nbData) = ones(obj.nKernel,1) * s(n).c(end); %Rescaling
		...
		s(n).GAMMA = (s(n).ALPHA.*s(n).BETA) ./ repmat(sum(s(n).ALPHA.*s(n).BETA)+realmin, obj.nKernel, 1); 
	...
	if obj.params_updateComp(4)
		obj.Trans = sum(ZETA,3)./ repmat(sum(GAMMA_TRK,2)+realmin, 1, obj.nKernel); 
	end
