function [traj] = SampleHSMMLQR_SC(obj, currP, N)
%SampleHSMMLQR_SC Sampling based iterative LQR controller for HSMM
%   currP: DP x 1, the current/initial position
%   N: Integer, the num. of states required
%   --------------------------------------------------
%   traj: DP x N, the trajectory generated
%   @LfDHSMMZero
...
for i=1:K
    closeness(i,:)=obj.GaussPDF(currP, obj.Mu(:,i), obj.Sigma(:,:,i));
end
...
X = [currP; zeros(obj.DP,1)]; %[model.Mu(:,qList(1)); zeros(model.nbVarPos,1)];
r.X0 = X;
for t=1:nbData
    r.Data(:,t) = X; %Log data
