...
for i= 1:numSamples
   ...
   viaPoint_pos_i(:,end) = randn(1,2) .* 0.3 + viaPoint_pos_i(:,2)';
...
initPos = zeros(numSamples,4);
tic
for i = 1:numSamples
    for j = 1:1e4
        err = squeeze(tr_des(i, 1,:)) - environment.getForwardKinematics(initPos(i,:))';
        if ( norm(err) < 1e-2 ) 
            break;
        end
        vel = 1e-2 * environment.getJacobian(initPos(i,:))' * err;
        initPos(i,:) = initPos(i,:) + vel';
    end
    if ( j == 1e4 )
        keyboard
    end
