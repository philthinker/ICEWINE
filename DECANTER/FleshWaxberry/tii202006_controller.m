...
controller_pre.K(1:3,1:3) = 200 * eye(3);
controller_pre.K(4:6,4:6) = sqrt(200) * eye(3);
controller_pre.D(1:3,1:3) = 100 * eye(3);
controller_pre.D(4:6,4:6) = sqrt(100) * eye(3);
% Retrieve one by one
timeout = 100;
...
for m = 1:M
    ...
    p_t = hat_p(:,1); q_t = hat_q(:,1); dp_t = zeros(3,1); omega_t = zeros(3,1); ddp_t = zeros(3,1); domega_t = zeros(3,1);
