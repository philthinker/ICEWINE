classdef FWTPGMM2 < FWTPGMM
    %FWTPGMM2 Frame-weighted TP-GMM with inverse alpha
    %   Just a test for figure
    %   Haopeng Hu
    %   2020.07.25
    %   All rights reserved
    ...
    methods
        function obj = FWTPGMM2(K,D,F)
            %FWTPGMM2
            obj = obj@FWTPGMM(K,D,F);
        end
        ...
        function [expData,expSigma,alpha] = GMR(obj,query,qFrames)
            ...
            for m=1:F
                ...
                for t=1:N
                    ...
                    SigmaGMRinv(:,:,t,m) = SigmaGMR(:,:,t,m)\eye(Dout);
                end
            end
...
%                     SigmaTmp(:,:,t,m) = qFrames(m).A(Din+1:end,Din+1:end) * SigmaGMR(:,:,t,m) * qFrames(m).A(Din+1:end,Din+1:end)';
                    ...
                    tmpWeigths = 0;
                    for f = 1:F
                        tmpWeigths = tmpWeigths + norm(SigmaGMRinv(:,:,t,f));
                    end
                    alpha(m,t) = norm(SigmaGMRinv(:,:,t,m))/tmpWeigths;
