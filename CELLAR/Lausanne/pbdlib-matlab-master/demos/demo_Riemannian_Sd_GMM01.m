...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
x = [randn(model.nbVar,nbData/2).*3E-1 - 1, randn(model.nbVar,nbData/2).*3E-1 + .6];
for t=1:nbData
	x(:,t) = x(:,t) ./ norm(x(:,t));
end
u = logmap(x, [0;1]);
...
figure('position',[10,10,1000,1000],'color',[1 1 1]); hold on; axis off; 
plot(0,0,'k+');
plot(cos(tl), sin(tl),'-','color',[.8 .8 .8]);
plot(x(1,:),x(2,:),'k.','markersize',10);
for i=1:model.nbStates
	plot(model.MuMan(1,i), model.MuMan(2,i), '.','markersize',30,'color',clrmap(i,:));
