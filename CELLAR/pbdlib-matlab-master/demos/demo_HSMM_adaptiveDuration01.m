function demo_HSMM_adaptiveDuration01
% Hidden semi-Markov model with adaptive duration, used with a controller  
% based on discrete finite-horizon LQR updated online (position and velocity 
% tracking with augmented state representation).
%
...
X = [Data0; 1];	%Initial state vector

for t=1:nbData
	if mod(t,10)==1
		fprintf('.');
	end
	r(1).Data(:,t) = X(1:end-1); %Log position
	for i=1:model.nbStates
...
for i=1:model.nbStates
	plotGMM(model0.Mu(1:2,i), model0.Sigma(1:2,1:2,i), clrmap(i,:), .6);
end
...
for t=1:1:nbData
	...
	%Spatial plot of the data
	subplot(3,4,[1,5,9]); hold on;
	h1 = plot(r(1).Data(1,t), r(1).Data(2,t), 'o','lineWidth', 2.5, 'color', [0.9 0.6 0.3]);
	h1 = [h1 quiver(r(1).Data(1,t), r(1).Data(2,t), r(1).ddx(1,t)*3E-4, r(1).ddx(2,t)*3E-4, 25, 'LineWidth', 2, 'color', max(min(h(:,t)'*clrmap,1),0))];
	axis([-10 10 -10 10]); 

	subplot(3,4,2:4); hold on;
	for i=1:model.nbStates
		yTmp = r(1).Pd(i,:,t) / max(r(1).Pd(i,:,t));
		h1 = [h1 patch([1, 1:size(yTmp,2), size(yTmp,2)], [0, yTmp, 0], clrmap(i,:), 'LineWidth', 2, 'EdgeColor', clrmap(i,:), 'facealpha', .6, 'edgealpha', .8)];
	end
	axis([1 nbData 0 1]);
	...
	for i=1:model.nbStates
		h1 = [h1 patch([1, 1:t, t], [0, h(i,1:t), 0], clrmap(i,:), 'LineWidth', 2, 'EdgeColor', clrmap(i,:), 'facealpha', .6, 'edgealpha', .8)];
	end
...
for i=1:2
	...
	for t=1:nbData-1
		if size(msh,2)==0
			msh(:,1) = [t; model0.Mu(i,qList(t))];
		end
		if t==nbData-1 || qList(t+1)~=qList(t)
			%Reference
			msh(:,2) = [t+1; model0.Mu(i,qList(t))];
			%Variance
			sTmp = model0.Sigma(i,i,qList(t))^.5;
