function demo_trajHSMM_adaptiveDuration_online01
% Online trajectory retrieval method built on an HSMM with adaptive duration 
% and a trajectory-GMM representation.
%
...
model.dt = 1; %Time step (large values such as 1 will tend to create clusters by following position information)
...
% Construct operator PHI (big sparse matrix), see Eq. (14) in Rozo et al. 2016, Frontiers in Robotics and AI
...
% 				oTmp = prod(c(1:t) .* gaussPDF(r(1).Data(:,1:t), model.Mu(:,i), model.Sigma(:,:,i))'); %Observation probability for standard HSMM
	...
	invSigmaQ = zeros(length(qList)*model.nbVar, length(qList)*model.nbVar);
	for tt=1:length(qList)
		id1 = (tt-1)*model.nbVar+1:tt*model.nbVar;
		MuQ(id1,1) = model.Mu(:,qList(tt));
		invSigmaQ(id1,id1) = inv(model.Sigma(:,:,qList(tt)));
	end
...
%% Animated plot
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
hf = figure('position',[10,10,2300,900],'color',[1 1 1]);
...
%plot(r(1).TarData(1,:), r(1).TarData(2,:), '--','lineWidth', 2.5, 'color', [0.3 0.3 0.3]);
plot(r(1).Data(1,:), r(1).Data(2,:), '-','lineWidth', 2.5, 'color', [0.2 0.2 0.8]);
...
for t=1:1:nbData
	...
	h1 = []; %plot(r(1).TarData(1,t), r(1).TarData(2,t), 'x','lineWidth', 2.5, 'color', [0.6 0.9 0.3]);
